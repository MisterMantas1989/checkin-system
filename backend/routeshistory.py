...
def history():
    ...
    try:
        ...
        excel_entries = df[(df['Namn'].str.strip() == namn) & (df['Mobil'] == mobil)]
    ...
    # 2. PostgreSQL-data
    try:
        db_rows = Checkin.query.filter_by(user=namn, mobil=mobil).order_by(Checkin.checkin_time.desc()).all()
        for row in db_rows:
            entries.append({
    ...
    def get_sort_key(e):
        return e.get('checkin_time') or f"{e.get('Checkin-datum', '')} {e.get('Checkin-tid', '')}"
